...
class MenuAdapter(
    ...
    inner class PrimaryViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: MenuItemData) {
            val bgColor = ContextCompat.getColor(itemView.context, bgColorRes(item.colorScheme))
            val textColor = MaterialColors.getColor(itemView, onContainerAttr(item.colorScheme), 0)
            ...
            if (lockIcon.visibility == View.VISIBLE) lockIcon.setColorFilter(textColor)
    ...
    inner class ActionViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: MenuItemData) {
            ...
            if (item.showArrow) arrowView.setTextColor(textColor)
