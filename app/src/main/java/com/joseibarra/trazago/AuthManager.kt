...
object AuthManager {
    ...
    private const val ADMIN_CACHE_TTL_MS = 5 * 60 * 1000L
    private var cachedAdminStatus: Pair<Boolean, Long>? = null
    ...
    suspend fun isAdmin(): Boolean {
        ...
        val cached = cachedAdminStatus
        if (cached != null && System.currentTimeMillis() - cached.second < ADMIN_CACHE_TTL_MS) {
            return cached.first
        }
        return try {
            val result = user.getIdToken(false).await()
            val isAdmin = result.claims["admin"] as? Boolean == true
            cachedAdminStatus = Pair(isAdmin, System.currentTimeMillis())
            isAdmin
        } catch (e: Exception) {
            false
        }
