        ...
        if (geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
            ...
            if (triggeringGeofences != null) {
                for (geofence in triggeringGeofences) {
                    ...
                    Log.d(TAG, "User entered geofence for: $requestId")

                    if (requestId.startsWith("event_")) {
                        val eventId = requestId.removePrefix("event_")
                        val eventIntent = Intent(context, EventDetailsActivity::class.java).apply {
                    ...
                    proximityManager.showProximityNotification(requestId, "lugar turístico")
                }
            }
        }
    }
