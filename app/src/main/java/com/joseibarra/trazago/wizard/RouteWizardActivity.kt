...
import com.joseibarra.trazago.databinding.ActivityRouteWizardBinding
import com.joseibarra.trazago.model.GeneratedRoute
import com.joseibarra.trazago.result.RouteResultActivity
 ...
 */
class RouteWizardActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportActionBar?.title = getString(R.string.wizard_plan_title)
    ...
    private fun updateUI(position: Int) {
        ...
        val stepLabels = arrayOf(
            getString(R.string.wizard_step1_label),
            getString(R.string.wizard_step2_label),
            getString(R.string.wizard_step3_label),
            getString(R.string.wizard_step4_label)
        )
        binding.tvStepLabel.text = getString(R.string.wizard_step_format, position + 1, totalSteps, stepLabels[position])

        // Botón siguiente / generar
        binding.btnNext.text = if (position == totalSteps - 1) "" else getString(R.string.wizard_next)
        binding.btnNext.visibility = if (position == totalSteps - 1)
            android.view.View.GONE else android.view.View.VISIBLE

        // Botón atrás
        binding.btnBack.text = if (position == 0) getString(R.string.wizard_cancel) else getString(R.string.wizard_back)
    }
    ...
    private inner class WizardPagerAdapter(activity: androidx.appcompat.app.AppCompatActivity) :
